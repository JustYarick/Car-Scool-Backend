...
import com.kubgtu.car_school.model.responces.RequestResponse;
...
public class RequestController {
    ...
    public ResponseEntity<RequestResponse> updateRequest(@Valid @RequestBody UpdateUserRequestRequest updateUserRequestRequest) {
        return ResponseEntity.ok(new RequestResponse(requestService.update(updateUserRequestRequest)));
    }
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public ResponseEntity<RequestResponse> getRequest(@Valid @PathVariable Long id) {
        return ResponseEntity.ok(new RequestResponse(requestService.getById(id)));
    }
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public ResponseEntity<RequestResponse> getRequests(@RequestParam(required = false, defaultValue = "0") int page,
                                                            @RequestParam(required = false, defaultValue = "100") int size) {
        return ResponseEntity.ok(RequestResponse.builder()
                .data(requestService.getByPage(page, size))
                .build()
        );
    }
}